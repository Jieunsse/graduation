...
export interface DriverRanking extends DriverRankingBase {
  englishName: string;
}
...
const rawDriverRankingData: DriverRankingBase[] = [
  {
    id: 'lando-norris',
    name: '랜도 노리스',
    code: 'LN',
...
export const driverRankingData: DriverRanking[] = rawDriverRankingData.map((entry) => ({
  ...entry,
  englishName: toEnglishName(entry.id),
}));

export const driverShowcaseLookup = driverRankingData.reduce(
  (acc, entry) => {
    acc.set(entry.englishName.toLowerCase(), entry);
    acc.set(entry.code.toLowerCase(), entry);
    acc.set(entry.id.toLowerCase(), entry);
    return acc;
  },
  new Map<string, DriverRanking>()
);

export const teamLogoLookup = driverRankingData.reduce(
  (acc, entry) => {
    if (!acc.has(entry.teamId)) {
      acc.set(entry.teamId, entry.teamLogoUrl);
    }
    return acc;
  },
